...
MAX_ALLOWED_RADIUS_SENSOR = 60.0 # Was 10

SENSORS_LIMITS = {
    'sensor.camera.rgb': 8, #Was 4
...
class AgentWrapper(object):
    ...
    allowed_sensors = [
        ...
        'sensor.spectator',         # For debugging
        'sensor.other.gt_gnss',     # For debugging
        'sensor.other.gt_imu',      # For debugging
    ]
    ...
    def setup_sensors(self, vehicle, debug_mode=False):
        ...
        for sensor_spec in self._agent.sensors():
            ...
            # These are the sensors spawned on the carla world
                ...
                #Debug sensors
                if sensor_spec['type'].startswith('sensor.spectator'):
                    bp = bp_library.find('sensor.camera.rgb')
                elif sensor_spec['type'].startswith('sensor.other.gt_imu'):
                    bp = bp_library.find('sensor.other.imu')
                elif sensor_spec['type'].startswith('sensor.other.gt_gnss'):
                    bp = bp_library.find('sensor.other.gnss')
                else:
            ...
            # setup callback
            if sensor_spec['type'].startswith('sensor.spectator'):
                sensor.listen(CallBack(sensor_spec['id'], 'sensor.camera.rgb', sensor, self._agent.sensor_interface))
            else:
